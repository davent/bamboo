...
# - Checkout to a sub-directory: go/src/github.com/davent/bamboo
# - Post build action / Archive the artifacts: *.deb
# - Test output: test_output/tests.xml

export BAMBOO_PROJECT_DIR=$WORKSPACE/go/src/github.com/davent/bamboo
...
go build
go test -v github.com/davent/bamboo/... | go2xunit > $WORKSPACE/test_output/tests.xml
